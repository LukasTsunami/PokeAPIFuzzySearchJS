...
export class FuzzyPokemonSearch {
  ...
  async buscar({ criterioDeBusca, usarClausulaANDParaBusca = false, pagina = 1 }) {
    ...
    if (usarClausulaANDParaBusca) {
      resultados = Object.entries(criteriosTraduzidos).reduce((acumulado, [campo, valor]) => {
        const fuseBusca = campo === 'type' ? this.fuseType : new Fuse(acumulado, { keys: [campo], threshold: this.precisaoDaBusca });
